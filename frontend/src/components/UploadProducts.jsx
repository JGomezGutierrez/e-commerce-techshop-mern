...
const UploadProducts = ({ onClose, fetchData}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try{
    // Petición al backend
    const request = await fetch(Global.url + "product/upload-product", {
      method: "POST",
  ...
  //     icon: 'success',
  //     title: 'success',
  //     text: data.message || 'Producto cargado con éxito'
  //   });
  //   onClose()
  //   } else {
  ...
  // }
    ...
    if (request.ok && data.status === "success") {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: data.message || 'Error al subir el producto'
      });
    }
  } catch (error) {
    console.error('Error al cargar los productos al sistema:', error);

    Swal.fire({
      icon: 'error',
      title: 'Error',
      text: error.message || 'Error al subir el producto. Inténtalo de nuevo más tarde.'
    });
  }
  }
